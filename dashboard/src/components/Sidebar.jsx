...
// Remove the following line since `NavLink` is not used
import { NavLink } from 'react-router-dom';
// Remove the following line since `links` is not used
  ...
  return (
    <>
      <div className='ml-3 h-screen md:overflow-hidden overflow-auto md:hover:overflow-auto pb-10'>
        ...
        {links.map((item) => (
          <div key={item.title}>
                    <p className='text-gray-400 m-3 mt-4 uppercase' key={item.title}>
                      {item.title}
                    </p>
                    {item.links.map((link)=>(
                      <NavLink
